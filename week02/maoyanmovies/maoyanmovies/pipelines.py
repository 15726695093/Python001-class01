...
class MaoyanmoviesPipeline(object):
    ...
    def init_mysql(self, spider):
        ...
        # 创建表如果表不存在
        self.cur.execute("create table if not exists moyan_top(movie_name text, movie_time text, movie_type text)")


    # mysql插入数据
    def insert_mysql(self, item):
        try:
            self.cur.execute("insert into moyan_top(movie_name, movie_time, movie_type) values ('{}', '{}', '{}');".format(
